...
const Gauge: React.FC<GaugeProps> = ({
  ...
  const size = 200; // Reduced from 250 for better fit
  const center = size / 2;
  const radius = center - 15; // Adjusted for better proportions
  const angleStart = -225;
  const angleEnd = 45;
  const angleRange = angleEnd - angleStart;

  // Ensure value is within [0, max]
  const clamp = (val: number, min: number, mx: number) => Math.max(min, Math.min(val, mx));
  ...
  const pointerLength = radius - 10; // Adjusted for better fit
  ...
  for (let i = 0; i <= max; i += step) {
    ...
    const tx = center + (radius - 18) * Math.cos(rad);
    const ty = center + (radius - 18) * Math.sin(rad);
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Gauge label */}
      <div className="mt-1 text-sm text-gray-800 uppercase tracking-wide font-semibold">
        {label}
      </div>
    </div>
  );
